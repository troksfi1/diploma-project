...
V~rámci obrazovky \textit{Události} je používána stejná komponenta pro zobrazování událostí jako byla použita na domovská obrazovce. 
...
Tento přístup umožňuje snadnou konfiguraci závislostí a poskytuje
...
Tyto události jsou zpracovány aplikací pomocí metod, jako je například metoda \code{onClick()}, která je typickým příkladem pro UI komponenty. 

Tato metoda \code{onClick()} je možné uplatnit reprezentaci UI události v podobě datových objektů zadefinovaných ve výpisu kódu \ref{lst:EventsImpl}.
...
a mohou spustit různé akce, které by zároveň
...
zaručit, že stav uživatelského rozhraní nebude změněn v~případě 
