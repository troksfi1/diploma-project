...
\begin{figure}[H]
  \centering
  \includegraphics[width=.3\textwidth]{profile.png}
  \caption{Ukázka podoby UI dle kód z ukázky \ref{lst:ConsumeUIState}}
...
události patří. Pro každou z těchto kategorii poté vytvoří jeden \code{EventLazyRow}, do kterého umístí příslušné události.
...
rozmezí pomocí kliknutí na ikonu kalendáře. Po kliknutí na ikonu kalendáře se uživateli zobrazí takzvaný \code{DatePicker}, který umožňuje zvolit požadované časového rozmezí.
...
částmi aplikace. 
%Hlavním cílem navigace je poskytnout uživatelům intuitivní a plynulý způsob průchodu obsahu a samovolně zlepšit jejich provádění akcí v~aplikaci. 
...
například pomocí tlačítka zpět na platformě Android nebo jakéhokoliv tlačítka zpět implementovaného v~rámci aplikace. \textit{Stack API}
 ...
 nebo funkce \code{pop}, \code{popAll}, \code{popUntil} pro zpětný návrat. \cite{voyagerStack}
...
Kromě těchto druhů navigace byla knihovna Voyager ještě použita k implementaci ViewModelů, které je možné díky integraci této knihovny
...
\begin{listing}[H]
\caption{Composable funkce \code{AsyncImage}poskytovaná knihovnou Coil}\label{lst:Coil3}
\begin{minted}{Kotlin}
  AsyncImage(
...
štítkem (viz štítek \code{selectNewestEvents:} ve výpisu kódu \ref{lst:SQLDotaz}), skrze který je vygenerována příslušná Kotlin funkce. 
...
I~přesto, že se jedná o~multiplatformní knihovnu a většina kódu může být psána v~balíčku sdíleném mezi všemi platformami, je zde i 
...
\subsubsection*{Koin}
Koin je moderní framework pro vkládání závislostí (zkráceně DI - z~anglického \uv{dependency injection}) \cite{Koin}. Jedná se o~lehkou a snadno 
...
společné třídy pracující s databází (\code{DatabaseDaoImpl(databaseDriverFactory: DatabaseDriverFactory)}).
...
V rámci takovýchto metod je pak vhodné implementovat reprezentaci UI události například v podobě datových objektů 
