...
syntaxí jako je používaná pro ostatní části projektu. 
...
sourceSets podle toho, zdali se jedná o~multiplatformní (\code{commonMain}) nebo nativní závislost (\code{androidMain}, \code{iosMain}, atd.).
...
Jelikož se jedná o~styly písma přímo určené systémem Material Design, tak lze k~těmto stylům přistupovat přes
...
které jsou aktuálně viditelné, čím nezpomaluje načítání obsahu a v~případně většího množství událostí a výrazně šetří zdroje potřebné k~vykreslení.
...
\code{com.google.maps.android.compose}, která umožňuje vložení mapových podkladů na platformě Android a k propojení této nativní části 
...
částmi aplikace. Hlavním cílem navigace je poskytnout uživatelům intuitivní a plynulý způsob průchodu obsahu a intuitivně zlepšit jejich provádění akcí v~aplikaci. 
...
V~případě implementované aplikace mají ViewModely jasně definovanou roli a to zpracovávat události přicházející z~UI na jejich základě 
...
je zapotřebí dodat příslušnou instanci úložiště (například \code{RepositoryImpl}) a následně již je možné implementovat příslušné funkce
