...
s~technologií Compose Multiplatform. Kapitola dále shrnuje jejich podstatné rysy a následně slouží
...
mezi vybranými TODO frameworky byla zvolena 
...
Pro ještě snazší start vývoje poskytuje React Native možnost Expo framework, který 
...
Tyto dva celky byly spuštěny na souběžných vláknech a komunikovaly spolu pomocí zasílaných TODO serializovaných zpráv. Časem se ale ukázalo, že se tato komunikace
...
\cite{reactNativeRenderCommitMount} Ten se skládá z React Shadow Nodes, které reprezentují objekty v~C++, a tím přechází tato renderovací fáze do další fáze zvané commit.\cite{reactNativeRenderCommitMount}
...
Aplikace vytvořené pomocí frameworku Flutter jsou psány v~jazyce Dart - moderním a objektově orientovaném programovacím jazyce 
...
První z nich, embedder, je vrstva, která umožňuje integrovat Flutter do konkrétních platforem jako je Android, iOS, desktop nebo web. 
Každý embedder obsahuje platformě specifický kód, který je potřebný pro spuštění Flutteru na dané platformě.
Engine je další vrstva starající se o~vykreslování grafiky, kdykoli, kdy je potřeba vykreslit nový snímek. \cite{flutterArchOverview} 
...
Mezi hierarchicky poslední a neméně důležitou část tohoto frameworku patří platformě specifické knihovny Material and Cupertino. Tyto knihovny
...
základních widgetů jako je \emph{Image} nebo \emph{Text}, taktéž layout widget zvaný \emph{Container} a \emph{Row} pro organizaci a 
...
V~porovnání s~ostatními dříve zmíněnými frameworky slouží pouze tento k~tvorbě multiplatformního UI. Z~toho důvodu je převážně 
...
komponentu, bez které by Compose Multiplatform nevznikl, je tomuto toolkitu věnována celá kapitola \ref{kmpSection}.
...
Android views pro platformu Android. Každopádně, od těchto něco... TODO je postupně upouštěno a přechází se k~deklarativním frameworkům jako je Jetpack Compose nebe SwiftUI.  
...
zvané Expected a actual deklarace. TODO
...
Aby se zdvojení zabránilo, je vhodné aplikovat dříve zmíněné hierarchické struktury a využít takzvané \textit{source sets}, které slouží 
...
který smí obsahovat pouze kód a případné knihovny. Tyto knihovny a kódy jsou multiplatformní a pokrývají tak veškeré platformy používané v~projektu.

TO DO A~dále na adresáře pro vybrané platformy jako například \code{AndroidMain}, \code{iosMain}, které již mohou být závislé na pouze platformě specifický knihovnách.
...
známých firem v~produkčním nasazení, jsou to například firmy Forbes či McDonald's. Forbes uvádí, že právě díky KMP dokáží sdílet až 80 \%
...
Mezi klíčové parametry, kvůli kterým většina firem upřednostňuje vývoj nativních aplikací, patří především jejich výkon. Z~toho důvodu se následující podkapitola věnuje právě porovnání výkonu a dalších parametrů multiplatformních aplikací
...
pro jiné platformy. \cite{} Kdežto u~velikosti iOS aplikace je situace výrazně odlišná. Samotná aplikace pro iOS je v~porovnání
s~její nativní aplikací o~23,1 MB větší. Tento rozdíl ve velikosti je způsoben především grafickou 2D knihovnou Skia,
která je na platformě Android dostupná, kdežto na platformě iOS se nenachází. Knihovna proto musí být spolu s~aplikací dodána. \cite{}
...
Co se týče plynulosti UI na platformě iOS, se momentálně nejvíce řeší problém s~rychlostí vykreslování.
...
se jedná o~veškeré zmíněné problémy kromě zařazení HarmonyOS mezi podporované platformy. % // todo 
V~tomto plánu je mimo jiné uvedeno, že aktuální prioritou číslo jedna je změna frameworku Compose Multiplatform na platformě iOS do verze Beta. \cite{KMPRoaddMap}
