...
Práce se zabývá otázkou proč se zabývat multiplatformním vývojem a co to vlastně multiplatformní vývoj je a jakých typů zařízení se může týkat.
Dále je probrána otazka kdy se vyplatí aplikovat multiplatformní UI a na jakých zařízeních. 

a porovnání vazby s UI s logikou daného aplikace vůči multipaltfomovodsti.


kded multipaltformí vývoj nemá smmysl (vyfocení závady)


Jaké jsou výhody a nevýhody multiplatformních aplikací  v porovnaní s nativními
aplikacemi.
...
V v jedné z posledních části se týká možýností testování takto vytovřeného UI a následného testování UI na naimplementované aplikaci.

je věnován shrnutí celého procesu implementace a vyzdvyžení naskytlých problémů a výhod oproti natinímu pripadne jinym multiplatformicm framwoerkum
...
aplikací na platformamy Android a iOS současně. 
To že tomu tak je je primarně díky hlavnímu důvodu vecoucí firmy k multiplatformícmu vývojí aplikací a tím je snížení nákladu na vývoj.

Výběr těchto platforem je vhodný především díky podobnostem majících na návrh multiplatformního UI vliv jako je napříkad velikost obrazovky.

I přesto, že multiplatformní farmeworky umožňují implementaci multiplatformním UI na většinu nejvíce používaných platofrm jajo je Android, iOS,
automotive, TV, wear OS, tak né vždy je těchnoto možností využít.

Z pohledu UI mají ty platformi často jíné velikosti obrazovek, jine moznožnosti ovládaní a proto, je často multiplatformní UI a často i veškerá logika
implementována konkrétně pro danou patformu nebo typ zařízení.
...
Obecně tedy nelze říci, že efektiva implementace multiplatformních aplikaci se může dle těchto požadavků výrazně lišit.
...
% to se tyka KMP ne compose Multipltfrom
...
Navržené drátové modely byly vytvořeny na základě funkčních požadavků a to tak, že ty nejdůležitější byly umístněny na domovkou
obrazovku.
...
comose jak je integrovan do aplikace

stejně tak jako na androidu, tak pro implementaci Com  byl použit sestavovancí nástroj Gradle, který 
...
Díky takzvaných \code{sourceSets} je možné pro každou platformu (balíček) implementovat platformě specifické závislosti. 
...
Voyager je multiplatformní navigační knihovna, která umožňuje navigovat mezi různými obrazovkami a destinacemi v multiplatformních mobilních
 aplikacích. Jeho hlavním cílem je poskytnout jednotné rozhraní pro navigaci v různých mobilních platformách, jako jsou Android a iOS, a 
...
který umožňuje konzistentní použití barev napříč aplikací.
Tento systém definuje sadu standardizovaných barev a pravidel pro jejich použití, což usnadňuje tvorbu uživatelského rozhraní a zajišťuje konzistentní vizuální styl.

Jelikož stránky města ... 
byl v aplikaci použita nejnovější volně dostupná verze design systému od společnosti Google zvaná Material 3.

loading


\subsubsection{Barvy}
Barvy byly zvoleny na základě vizuálního stylu města, které Příbram využívá při všech prezentaččních příležitostech.
...
Testování aplikací založených na frameworku Compose Multiplatform je stejně tak jako tvorba samotného UI založena Jetpack Compose a využívá 
proto i stejných principů. Compose Multiplatform z toto důvodů tatkéž využívá tři hlavní principy jak testovat UI, které se v Jetpack compose
označují jako \code{Finders}, \code{Assertions} a \code{Actions}. 

Finders umožnují

Pomocí Assertions 

A pomocí Actions je možné simulovat uživatelské integrace jako jsou kliknutí nebo jiná gesta. \cite{composeTesting}

Od verze 1.6.0 Compse multiplatform umožnuje testování UI na všech platformách. \cite{composeNews1.6.0}

Implementace UI testů je možná díky 
