...
Návrh se zaměřuje na navržení jednat grafické podoby aplikace tak techničtější části, které je pro správné fungování UI nezbytná.


skloubení případů užití s grafickou podobou UI, tak aby nejčastější uživatelovi cílem bylo možné splni s co nejmenší dávkou  usílí.
což obecně vede k intenzivnějšímu používaní a oblibě aplikace.

Navržení Use casu je doležite jednaz pohledu jaci uživatele budou danou aplikaci pouzivat tak z pohledu na jakých zařízení případně 
platform ji budou využívat.
...
Případy užití jsou klíčovým nástrojem pro pochopení potřeb uživatelů a návrh funkcionality aplikace tak, aby co nejlépe odpovídala těmto potřebám. 
...
Na základně...byla pro události vybrána možnost zobrazení jednotlivých událostí v postupně skrolovacím řádku a to především z důvodu,
 že akce poutají především svým vizuálním největší část informace he uživateli předána právě pomocí titulního obrázku, názvu
a času konání.

Oproti tomu pro seznam novinek byl vybráno uspořádnaní novinek do scrolovancího sloupe, který lépe umožňuje vyžít celou šíři 
obrazovky k zobrazení delšěích titulků a textů událostí.
...
V rámci obrazovky "Události" je použit stejný systém pro zobrazené údálostí pomocí skrolovaního radku

Oproti domovské stránce jsou zde zobrazeny veškeré události, které se ve městě budou konat. Pro lepší přehlednou jsou udáslosti rozděleny 
do kategorií 
...
Jelikož je ale aplikce cílena pro široké spektrum lidí, byl zde přidám, který uživatelům umožňuje přízpůsobení domovské obrazovky.
...
S ohledem na to, že aplikce je navrhována pro použití na mobilních zařízením, tak je tomu uzpůsoben i návr samotné design systému, který
specifikuje především prvky použitelné na mobilních platformách a zbylé prvky mohou být případně v budoucnu rozšířený o další prvky 
používané na jiných platformách.

Compose multiplatform je přípraven na implementaci design systému. 

Navržení se proto týká především těmto částém,
které mohou být v budoucnu rozšířený o další prvky používané na jiných platformách.


jehož byla aplicace designována ne

byl celý návrh designován nemá nikde explicitně definovaný design systém. byl proto navržen vlastní a jeho tvorba 

Pro návrh grafické podoby UI byly použity prvky používané  barvy používané v rámci 

\section{Návrh mockup modelu}

Po zadefinování použitelných komponet v navženém design systému \ref{} jež samotný návrh grafické podoby UI zjednodušen nezbytné minimum.
...
Byl vytvořen obecný design systém pro tvorbu UI, který urychlí proces navrhovaní grafické podoby UI a při následné fázi implementace zajistí intuitivní
ovládání aplikace a zaručí konzitentní vzhled napříč aplikací a případně i dalšími aplikacemi města. Zároveň poslouží k tvorbě ostatních 
aplikací města.
...
Při implementace design systému pomocí Compose Multiplatfom byla snaha co nejpřesněji napodobit navržený design sytém z kapitoli \ref{}
...
jednotlivé komponenty odpovídali prvkům vytvořeného design systému. 

LAZY ROW LAZY COLLUMN , OBRZKY POCI COIL, STLOVANI POCI DESIGN SYTEMU, VYUZITI DATABAZE, ukazka Event lazy row komoponenty
...
V rámci obrazovky "Události" je použí vána stejená koomponanta, pro zobrazené údálostí "EventLazyRow". 

LAZY ROW , DATE PICKER, SPECIAL CHIPS


\subsection{Parkování obrazovka}
Jak jž bylo zmíněno v kapitole \ref{} tak implementace matových podkladů byla implementována pomocí 
...
Dále byla tato obrazovka rozšířena o vyhladvací bar, který uživateli dává možnost jednoduchého vyhledání parkovací zóny podle názvu.
